...
class AmazonBotSpider(CrawlSpider):
    ...
    def parse_item(self, response):
        ...
        if Link==[]:
            ...
            if Link!=[]:#Sección "detalles del producto"
                ...
                '''
                ml_item['asin'], ml_item['rank'], ml_item['categoria'], ml_item['valoracion_media']=self.get_details(Link[0])
            else:#No existe esta sección, no podemos obtener estos datos
                
                print(Link)
                if Link!=[]:
                    ml_item['asin'], ml_item['rank'], ml_item['categoria'], ml_item['valoracion_media']=self.get_details3(Link[0])
                else:
            ...
            '''
            ml_item['asin'], ml_item['rank'], ml_item['categoria'], ml_item['valoracion_media']=self.get_details2(Link[0])
        if self.marketplace=='it' and ml_item['categoria'] != '':
            ml_item['categoria']=ml_item['categoria'][2:]
    ...
    def get_details(self, details):
        ...
        return asin, rank, categoria, valoracion_media
